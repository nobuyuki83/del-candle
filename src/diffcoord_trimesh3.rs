...
fn test_backward() -> anyhow::Result<()> {
    let (tri2vtx, vtx2xyz0) = del_msh_core::trimesh3_primitive::sphere_yup::<u32, f32>(
        0.8, 32, 32);
    let vtx2vtx = del_msh_core::vtx2vtx::from_uniform_mesh::<u32>(
        &tri2vtx, 3, vtx2xyz0.len()/3, false);
    ...
    let (loss0, dw_vtx2xyz) = {
        let render = Layer {vtx2idx: vtx2idx.clone(), idx2vtx: idx2vtx.clone()};
    ...
    for i_vtx in 0..num_vtx {
        for i_dim in 0..3 {
            ...
            assert!((val_num-val_ana).abs()<0.01*(val_ana.abs()+0.1), "{} {}", val_ana, val_num);
        }
    }
    Ok(())
}
